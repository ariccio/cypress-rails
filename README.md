...
* **CYPRESS_RAILS_OVERRIDE_FULL_BASE_PATH** NOTE TO SELF (@ariccio): does not seem needed??! works witout it now?
  There are ugly use cases that are least unreasonably handled by feeding a different port and base path to the CYPRESS_BASE_URL than to the CYPRESS_RAILS_PORT.
